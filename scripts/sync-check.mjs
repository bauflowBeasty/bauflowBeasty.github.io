...
if (!existsSync(estado.plasticCli)) {
  ...
  try {
    ...
    const cs = salida.split(/\r?\n/).filter((l) => /Assets[\\/]BeastyComponents/.test(l) && /\.cs\b/.test(l));
    if (!cs.length) bien('sin cambios .cs pendientes de check-in en Assets/BeastyComponents');
      ...
      for (const l of cs.slice(0, 30)) console.log(`     ${l.split('|').pop().trim()}`);
...
// ── 4: frescura del índice ───────────────────────────────────────────────────
seccion('3. Frescura de docs/DOC-INDEX.md');
