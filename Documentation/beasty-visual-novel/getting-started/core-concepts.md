...
Read this after [Your first scene](your-first-scene.md), when you have seen the pieces move.
...
self-contained: a fight, a flashback, a minigame's dialogue. See [Subgraphs](../authoring/subgraphs.md).
...
Right-click the graph canvas to create any of them. See [The story graph](../authoring/story-graph.md).
...
Full list with every field: [Blocks reference](../authoring/blocks-reference.md).
...
2. **Everything is saved automatically.** A save captures the store. That is one operation, and it picks
   ...
   See [Saving and loading](../production/saving-and-loading.md).
...
Full detail: [Variables and conditions](../world/variables-and-conditions.md), and
[Variable keys](../reference/variable-keys.md) for every reserved namespace.
...
that works rather than a promise. See [Controllers](../scripting/controllers.md) if you want to reach
into it from code.
...
- [The story graph](../authoring/story-graph.md) — the canvas and the seven node types
...
- [Scripting overview](../scripting/overview.md) — the same model, from code
