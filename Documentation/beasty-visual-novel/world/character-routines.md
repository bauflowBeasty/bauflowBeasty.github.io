...
Routines need [game time](game-time.md). With no Time Config assigned to the BeastyManager, every time
...
1. **The quest step's marker sprite**, when a [quest routine override](quests.md) moved her there and that step
   defines a marker.
...
[Free-roam rooms](free-roam-rooms.md) for how to add one.
...
a specific look in a specific room. See [Interactables and doors](interactables-and-doors.md).
...
2. otherwise the character's **[talk menu](talk-menu.md)**, if it resolves to any entries;
...
See [Character screens](character-screens.md).
...
2. **Rooms.** In the **FreeRoam** tab, create the rooms `Bakery` and `Park` (each gets its prefab; see
   [Free-roam rooms](free-roam-rooms.md)).
...
- [Game time](game-time.md) — dayparts, the clock, and how time moves.
- [Free-roam rooms](free-roam-rooms.md) — rooms, room prefabs, character spots.
- [Interactables and doors](interactables-and-doors.md) — character poses.
- [Quests](quests.md) — quest routine overrides.
- [Character screens](character-screens.md) — the in-game routine calendar.
- [Gameplay APIs](../scripting/gameplay-apis.md) — `BeastyRoutines` for programmers.
