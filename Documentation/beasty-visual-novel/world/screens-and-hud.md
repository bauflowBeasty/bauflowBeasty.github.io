...
| `AdvanceTime` | Changes the game clock, with the same operations as the [Advance time](game-time.md) block. A "wait" or "sleep" button. It needs a Time Config on the BeastyManager. |
...
- [UI prefabs](../production/ui-prefabs.md) — restyling every screen, and upgrading the shipped prefabs.
- [Character screens](character-screens.md) — the ready-made cast list, profile, stats, calendar and quest log.
- [Items and inventory](items-and-inventory.md) — the ready-made inventory screen.
- [Variables and conditions](variables-and-conditions.md) — what a live label and a case condition can read.
- [Game time](game-time.md) — the AdvanceTime action's operations.
