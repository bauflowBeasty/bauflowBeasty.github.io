...
- **A flow exit.** The option leaves the visual novel entirely: `Go to FreeRoam`, `Return to previous room`,
  ...
  it goes instead. See [Transitions](transitions.md).
...
- [Variables and conditions](../world/variables-and-conditions.md) — the full treatment: scopes, value types,
  every reserved key.
- [The story graph](story-graph.md) — the nodes and their ports.
- [Transitions](transitions.md) — flow exits as the target of a choice or a branch.
- [Blocks reference](blocks-reference.md) — the Set variable block.
