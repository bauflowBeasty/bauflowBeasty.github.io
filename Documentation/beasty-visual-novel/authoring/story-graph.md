...
![The Story tab: the Add blocks panel, the graph canvas and the node inspector](../images/vn-story-tab.png)
...
- The **Add blocks** panel on the left. It fills the selected node with instructions. See
  [Blocks reference](blocks-reference.md).
...
| `Create Subgraph` / `Open Subgraph` | Nests a graph inside this node, or drills into the one it already has. See [Subgraphs](subgraphs.md). |
...
Everything a dialogue node can contain is on the [Blocks reference](blocks-reference.md), and the two blocks
you will use most are covered in depth in [Dialogue and the stage](dialogue-and-stage.md).
...
[Choices and decisions](choices-and-decisions.md).
...
covered in [Transitions](transitions.md).
...
Add a route with the `+ outcome` button on the node. Full walkthrough in [Subgraphs](subgraphs.md).
...
A return node only makes sense inside a subgraph. See [Subgraphs](subgraphs.md).
...
The menu itself is authored on the character, not here. See [The talk menu](../world/talk-menu.md).

## See also

- [Blocks reference](blocks-reference.md) — every block a dialogue node can hold.
- [Dialogue and the stage](dialogue-and-stage.md) — writing a line, dressing the scene.
- [Choices and decisions](choices-and-decisions.md) — branching, conditions and effects.
- [Subgraphs](subgraphs.md) — nesting a graph and routing its outcome.
- [Transitions](transitions.md) — leaving the novel.
- [Core concepts](../getting-started/core-concepts.md) — project, context, graph, node, block.
