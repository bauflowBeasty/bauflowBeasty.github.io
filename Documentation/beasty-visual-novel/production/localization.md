...
2. **The operating-system language**, but only if `autoDetectSystemLanguage` is on in
   [VN Settings](vn-settings.md) AND the UI table actually has that language — so the game never starts in a
...
- [VN settings](vn-settings.md) — the default language, OS auto-detect, and where the UI table is assigned.
- [Validation and ids](validation-and-ids.md) — the validator reports keys with no source text, and the
  missing/stale totals.
- [Dictionary](../world/dictionary.md) — player-editable `[token]` text, which is a different mechanism.
- [Large projects](large-projects.md) — keeping a big table manageable.
