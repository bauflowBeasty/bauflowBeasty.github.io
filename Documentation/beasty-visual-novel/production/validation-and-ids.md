...
- **Localization.** Keys with no text in the source language, the totals of missing and stale translations, and
  keys the table holds that nothing references any more. See [Localization](localization.md).
...
- Stale generated code: `VNVars` and `VNChars` are rebuilt, so a variable or character you deleted stops
  existing in your C# too (see [Generated accessors](../scripting/generated-accessors.md)).
...
[UI prefabs](ui-prefabs.md).

## See also

- [Localization](localization.md) — the missing/stale reports the validator surfaces.
- [UI prefabs](ui-prefabs.md) — the black-screen symptom auto-wire fixes.
- [Menu items reference](../reference/menu-items.md) — every menu item in one table.
