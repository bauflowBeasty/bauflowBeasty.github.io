...
| `custom` | Your own Custom app mode is active ([Custom mode](../scripting/custom-mode.md)). |
...
- **A room.** A room prefab's `FreeRoamRoom` carries a `musicOverride` queue. Enter that room and its music
  ...
  [Free-roam rooms](../world/free-roam-rooms.md).
...
- [Blocks reference](../authoring/blocks-reference.md) — every block, including the audio ones, field by field.
- [Free-roam rooms](../world/free-roam-rooms.md) — where a room's music override lives.
- [UI prefabs](ui-prefabs.md) — the mixer prefab and the preferences screen with the volume sliders.
