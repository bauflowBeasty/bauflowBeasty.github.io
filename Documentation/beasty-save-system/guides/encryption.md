...
Two fields on [BeastySaveSettings](settings.md):
...
[Slots and metadata](slots-and-metadata.md). It also means metadata is the wrong place for anything you
...
- [Settings](settings.md) — every field, including `Encrypted` and `EncryptionKey`
- [Slots and metadata](slots-and-metadata.md) — why `meta` stays in plain text
- [Backups and corruption](backups-and-corruption.md) — what a `Corrupt` result means
- [Results and errors](../reference/results-and-errors.md) — `DecryptFailed` and the rest
- [The save file format](../reference/save-file-format.md) — where the ciphertext sits in the file
