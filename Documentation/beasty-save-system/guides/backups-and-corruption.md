...
[BeastySaveSettings](settings.md), which defaults to `true`.
...
thing to try. See [Results and errors](../reference/results-and-errors.md) for the full list.
...
- `DecryptFailed` — the save and the game disagree about encryption. See [Encryption](encryption.md).

## Doing it from the editor

The [Save Manager window](save-manager-window.md) lists every slot on disk with a **Restore Backup**
...
- [Slots and metadata](slots-and-metadata.md) — listing slots, and showing a damaged one in the UI
- [Settings](settings.md) — the `Backup` field
- [Results and errors](../reference/results-and-errors.md) — every error code and what causes it
- [The save file format](../reference/save-file-format.md) — the envelope and the checksum
