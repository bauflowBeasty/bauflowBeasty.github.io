...
One field controls it: `Strict` in [BeastySaveSettings](settings.md). It defaults to `true`.
...
[Versioning and migrations](versioning-and-migrations.md). Migrations preserve data. Tolerant loading
...
- [Settings](settings.md) — the `Strict` field
- [Versioning and migrations](versioning-and-migrations.md) — how to keep an old value instead of skipping it
- [What gets saved](what-gets-saved.md) — which fields are written in the first place
- [Results and errors](../reference/results-and-errors.md) — `FieldMapFailed` and the rest
- [Scene state](scene-state.md) — `LoadAll`, saveable ids and the scene-wide rollback
