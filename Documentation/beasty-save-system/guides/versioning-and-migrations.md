...
`JsonNode` is the package's own JSON node type — see [The JSON engine](../reference/json-engine.md) for
...
[Custom converters](../advanced/custom-converters.md).
...
editing a structure the package owns; read [The save file format](../reference/save-file-format.md) first
...
- [Settings](settings.md) — the `DataVersion` field
- [Strict vs tolerant loading](strict-vs-tolerant.md) — the other way to survive a changed class, and why it loses data
- [The JSON engine](../reference/json-engine.md) — everything `JsonNode` can do
- [The save file format](../reference/save-file-format.md) — where `dataVersion` lives in the file
- [Results and errors](../reference/results-and-errors.md) — `VersionTooNew` and `MigrationFailed`
- [Custom converters](../advanced/custom-converters.md) — the same Play Mode reset, for converters
