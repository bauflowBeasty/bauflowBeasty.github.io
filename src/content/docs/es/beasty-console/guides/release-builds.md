...
BeastyConsole.LogTrace($"Path recalculated for {agent.name}, {path.corners.Length} corners");
```
...
```csharp
if (debugPathfinding)
    BeastyConsole.LogTrace($"Path: {string.Join(" -> ", path.corners)}");
```
...
Consulta [Logging](/es/docs/beasty-console/guides/logging/).
...
Beasty Console no abre archivos, no rota logs y no sube nada. Llama a `UnityEngine.Debug`. La
...
plano. Consulta [Logging](/es/docs/beasty-console/guides/logging/).
