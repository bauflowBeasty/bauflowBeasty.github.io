---
title: "Controladores"
description: "Los tres MonoBehaviours que llamas desde código: BeastyManager , VNGameController , VisualNovelController . Todos en Beasty.VN.Runtime."
---
...
| Tarea | Llama a |
|---|---|
| Alcanzar cualquier manager | `BeastyManager.Instance` |
...
| Guardar o cargar una ranura orientada al jugador | `VNGameController` |
...
Regla general: `VNGameController` decide QUÉ se está ejecutando, `VisualNovelController` lo ejecuta, `VN` habla
con ello.
...
Accesores de managers, todos re-resueltos en tiempo de ejecución si una referencia se pierde:
...
Más las piezas de escena y la configuración que posee: `StageRoot`, `MainCanvas`, `MainCamera`, `Loading`,
...
despertado, y luego mantiene el overlay arriba hasta que cada barrera registrada devuelve true — o hasta que
transcurre `maxBootSeconds`, lo que ocurra primero. Luego oculta el overlay, establece `IsReady` y lanza
`Ready`.

Una barrera de arranque es cómo pones tu propio arranque lento dentro de esa ventana: una consulta remota de
configuración, un login, una migración de guardado, un calentamiento de tu propio sistema de assets. Sin una,
el jugador ve tus sistemas aparecer de golpe después de que el juego ya ha comenzado. Regístrala en `Awake` —
...
Limita la espera de forma realista. Una barrera que nunca devuelve true solo retrasa el juego en
...
overrides del diccionario. Se transporta a través de todos los modos, por eso el progreso sobrevive a un
cambio de proyecto y por eso todo dentro de él se guarda y se rebobina gratis.
...
- `PlayVisualNovelThenReturn` es el puente con FreeRoam: reproduce una escena, y luego vuelve a la sala que
  pasaste. El `roomBackground` opcional siembra el fondo, así que un diálogo sin su propio bloque Backdrop
  muestra la sala en la que el jugador está parado.
...
- `EnterCustom` entrega el juego a tu código. Ver [Modo personalizado](/es/docs/beasty-visual-novel/scripting/custom-mode/).
...
Las ranuras (slots) están nombradas: `manual_0`, `manual_1`, ..., `auto_0`, ... `LoadSlotDetailed` devuelve
...
principal, porque leer objetos de Unity lo requiere. Ver [Guardado asíncrono](/es/docs/beasty-save-system/guides/async-saving/)
para lo que eso realmente te da.
...
Para el panorama completo — política de autoguardado, miniaturas, qué contiene una ranura — ver
...
build anterior. Repórtalo; no lo trates como una carga exitosa.
...
`SharedVariableStore` es asignado por `VNGameController` al arrancar, con su propio `SharedVariables`. Cada
...
cuando la VN se reanuda. `StartNewGame` en el anfitrión llama a `PersistentStage.Reset()` para un inicio en
blanco.
