---
title: "Localización"
description: "Traduce un juego que ya está escrito. Esta página cubre las dos tablas de traducción, cómo el editor te indica qué traducciones quedaron desactualizadas, la importación"
---
...
idioma mientras el juego se ejecuta.

## Las dos tablas

Existen exactamente dos assets `LocalizationTable` en juego, y nunca se mezclan.

| Tabla | Dónde vive | Qué contiene |
|---|---|---|
| La tabla de historia | En el `VNContext` (`localization`), así que cada DialogueScene que comparte el contexto la comparte | Líneas de diálogo, etiquetas de elección, etiquetas del menú de conversación, texto de ítems y misiones — todo lo que la historia referencia por clave |
...
de esa clave en cada idioma. Por eso traducir es siempre solo "agregar una columna".
...
principio, así que el índice 0 sigue siendo el origen. Eliminar un idioma se lleva con él su columna de
traducciones.
...
más tarde editas la línea en inglés, la huella ya no coincide, y la celda que se tradujo a partir de la
...
- **Unused keys** — un alternador que lista las claves que la tabla contiene pero que ninguna línea de diálogo,
  elección o `[token]` referencia ya, con un barrido **Remove N unused key(s)**. Eliminar contenido de la
  historia deja sus claves atrás; así es como las limpias.
- **Fill identical texts** — el botón de memoria de traducción. Las líneas cuyo texto de origen es EXACTAMENTE
  ...
  una línea repetida como "Sí" una vez y rellena el resto. Nunca sobrescribe una celda que ya tiene texto.
...
La exportación usa el separador de lista del sistema en ejecución (una coma en en-US, un punto y coma en la
mayoría de las configuraciones regionales europeas), así que el archivo se abre directamente en columnas al
hacer doble clic, y se escribe como UTF-8 con BOM, así que los acentos sobreviven a Excel.
...
- Un archivo **sin fila de encabezado se rechaza** y no se escribe nada. Su primera línea es dato real, y
  leerla como encabezado la perdería silenciosamente.
- Reimportar una tabla completa **no borra tus marcas de desactualizado**: una traducción cuyo texto no cambió
  conserva su huella digital registrada. Solo una traducción cuyo texto realmente cambió se marca como fresca
...
que está actualmente en pantalla se vuelve a renderizar, las elecciones en pantalla se vuelven a renderizar, y
un menú de conversación abierto se vuelve a renderizar. El jugador puede cambiar de idioma en medio de una
línea y verla cambiar bajo sus ojos.

Cuando una clave no tiene texto en el idioma activo, se muestra en su lugar el texto del idioma de origen — una
celda vacía recae exactamente igual que una columna faltante, así que una línea sin traducir nunca se muestra
como una clave en bruto.
...
El selector **Lang** en la pestaña Story elige en qué columna de idioma estás ESCRIBIENDO mientras eres autor.
...
hace que el juego arranque en francés; `defaultLanguage` sí lo hace.
...
- [Proyectos grandes](/es/docs/beasty-visual-novel/production/large-projects/) — mantener manejable una tabla grande.
