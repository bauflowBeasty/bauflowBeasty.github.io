...
Si el jugador debe sentirse responsable de lo que pasa a continuación, usa una elección. Si la historia debe
simplemente ir por el camino correcto por lo que ya ha pasado, usa una decisión. Confundir esto es el error
de autoría más común, y se parece a un menú con una sola opción dentro.
...
- **Una salida de flujo.** La opción abandona la visual novel por completo: `Go to FreeRoam`,
  `Return to previous room`, `Choose room`, o `Go to another VN`. Una opción con una salida de flujo no
  tiene cable: la etiqueta del puerto muestra a dónde va en su lugar. Consulta
  [Transiciones](/es/docs/beasty-visual-novel/authoring/transitions/).

Eso significa que "Leave the tavern" y "Stay and ask another question" pueden estar lado a lado en el mismo
menú, una saliendo al mundo y otra continuando la escena.
...
El sprite también puede ser **condicional**, con el mismo modelo que un fondo de habitación: una lista
ordenada de casos, cada uno con una condición, y el primero que se cumpla gana. Si ninguno se cumple, se usa
el sprite por defecto. Así que la cara a la que estás respondiendo puede estar enfadada o contenta según lo
que hiciste antes, sin duplicar el nodo de elección.
...
Cuando la historia llega, el nodo recorre sus ramas de arriba abajo, toma la **primera rama cuya condición se
cumpla**, aplica los efectos de esa rama, y salta. Si ninguna rama coincide, toma el puerto
**`fallback →`**.
...
Las condiciones se definen del mismo modo en todas partes de Beasty VN: en una opción de elección, en una
rama de decisión, en un fondo de habitación, en una entrada de menú de conversación. Una condición es una
lista de cláusulas.
...
- El **valor** es con qué comparar.

### Combinar cláusulas

Las cláusulas se combinan con `And` u `Or`, y **AND liga más fuerte que OR**:

```text
a AND b OR c     is     (a AND b) OR c
```

Léelo como "o bien tanto a como b, o si no c". Si quieres la otra agrupación, divide la condición en dos
...
> *pretendías* rellenar y olvidaste se tragará la historia.
...
Los efectos existen en tres lugares dentro del alcance de esta página: en una opción de elección (aplicados
cuando el jugador la elige), en una rama de decisión (aplicados cuando se toma la rama), y en un nodo Return
(aplicados cuando un subgrafo devuelve el control). Escriben en el mismo almacén que el bloque
**Set variable**, así que persisten en las partidas guardadas y se retroceden correctamente.
...
- [Variables y condiciones](/es/docs/beasty-visual-novel/world/variables-and-conditions/) — el tratamiento completo: ámbitos, tipos de
  valor, todas las claves reservadas.
