...
description: "Síntoma, causa, solución. Encuentra tu síntoma, lee las dos líneas debajo, sigue el enlace si necesitas la historia completa."
---

Síntoma, causa, solución. Encuentra tu síntoma, lee las dos líneas debajo, sigue el enlace si necesitas la historia
completa.
...
sistema ya te ha dicho qué salió mal y tú no lo has leído.
...
segundo es **rechazado** y queda fuera del guardado por completo. Es silencioso en el juego, pero no es silencioso en
la consola.

**Solución.** Lee la consola. Se registra un error en el momento del registro nombrando el id. Luego pulsa **New** en
...
La forma habitual en que esto ocurre: duplicaste un GameObject que ya tenía un `BeastySaveable`, y la copia
se llevó el id consigo. Consulta [Estado de la escena](/es/docs/beasty-save-system/guides/scene-state/).
...
**Solución.** Una de tres:

- **Activa el módulo.** Cada módulo necesita su paquete de Unity presente en el proyecto (por ejemplo, el
  módulo Physics2D necesita `com.unity.modules.physics2d`). Instálalo y el módulo compila y se registra
  a sí mismo. Consulta [Módulos convertidores](/es/docs/beasty-save-system/reference/converter-modules/).
...
No tienes que esperar a que falle. El inspector de `BeastySaveable` te avisa en el editor: un componente marcado
sin convertidor se señala, y cada otro componente se etiqueta con la capa que lo convierte (`dev`, un id de módulo,
o `core`).
...
**Causa.** El checksum no coincide. El archivo se editó a mano, se truncó por un cierre inesperado, se dañó por el
disco, o se copió mal. El sobre (envelope) está lo bastante intacto para leerse; el payload no es lo que dice ser.
...
Un archivo corrupto nunca se rota hacia la copia de seguridad, así que el `.bak` es la última copia que se verificó. Consulta
[Copias de seguridad y corrupción](/es/docs/beasty-save-system/guides/backups-and-corruption/).

> **Nota**
> Un error `Corrupt` también se produce antes en la carga, para un archivo cuya forma del sobre es incorrecta — un
> documento JSON que se parsea pero no es un guardado de Beasty. La misma solución.
...
2. **La clave es incorrecta.** El `EncryptionKey` en `BeastySaveSettings` no es el que se usó para escribir el
   archivo. Ten en cuenta que un `EncryptionKey` vacío no significa "sin clave" — significa la clave por defecto compartida.
...
Las migraciones solo se ejecutan **hacia adelante**. El sistema puede actualizar un guardado antiguo a la versión actual; no
puede hacer bajar uno futuro.

**Solución.** En desarrollo, esto generalmente significa que un compañero de equipo escribió el archivo con su build, o que retrocediste tu proyecto.
Borra el slot, o sube `DataVersion` para que coincida.

En un juego publicado, significa que un jugador hizo un downgrade — un guardado en la nube de una versión más nueva
llegando a una instalación más antigua. Manéjalo: comprueba `BeastySaveError.VersionTooNew` y muestra "Este guardado se hizo con una
versión más nueva del juego", no un fallo genérico. Consulta
...
**Causa.** Casi seguro `FileNotFound`, y no comprobaste el resultado. Un slot faltante se registra como una
**advertencia, no un error** — deliberadamente, porque las pantallas de selección de slot sondean cada slot constantemente y
un error por cada slot vacío inundaría tu consola.
...
`BeastySave.Exists(slot, settings)` responde la pregunta directamente si eso es todo lo que necesitas. También comprueba
`BeastySaveManager.LastLoadResult` si estás usando el camino sin código `LoadAll`, y suscríbete a
...
**`Resources/`**. Si no se resuelve, la referencia ya conectada en la escena se deja intacta — así que el objeto
carga, y lleva puesto el sprite equivocado.
...
**Propiedades exclusivas del editor.** `Light.lightmapBakeType` es una API exclusiva del editor. El convertidor de `Light` la escribe
y la lee **solo en el editor**; un build ni la almacena ni la restaura. Un guardado hecho en el editor
lleva ese miembro, un build lo ignora — lo cual es correcto, y también por eso el tipo de bake de una luz
no sobrevive al player.

Además: `Light.cookie` se escribe por nombre pero **nunca se restaura**, y `MeshCollider.sharedMesh` nunca
se serializa en absoluto. Consulta [Módulos convertidores](/es/docs/beasty-save-system/reference/converter-modules/).
...
cada Play. Si los registraste desde un elemento de menú, un callback del editor, o un `Awake` de un MonoBehaviour que
no siempre se ejecuta, no están ahí cuando ocurre el guardado — y tus datos silenciosamente vuelven al
...
Los convertidores registrados con `BeastySave.RegisterModule` sobreviven al reinicio — por eso los módulos incorporados
siguen funcionando. Consulta [Convertidores personalizados](/es/docs/beasty-save-system/advanced/custom-converters/).

---

## Nada se guarda en WebGL

**Causa.** WebGL **no está soportado**. La escritura atómica depende de la semántica del sistema de archivos que el build de navegador
no tiene, y las variantes asíncronas están basadas en `Task`.

**Solución.** No hay ninguna, y no hay ninguna opción que lo cambie. Un build de navegador necesita una capa de
persistencia diferente. Consulta [Plataformas y límites](/es/docs/beasty-save-system/advanced/platforms-and-limits/).
...
| Error | Qué significa |
|---|---|
| `InvalidArgument` | Datos nulos, o un nombre de slot inválido. Los nombres de slot que intentan escapar de la carpeta, y los nombres de dispositivo de Windows, son rechazados. |
...
| `FieldMapFailed` | Un campo no se pudo mapear. En una carga estricta esto hace fallar toda la carga y **no se aplica nada**. En una carga tolerante habría sido una advertencia en su lugar. |
