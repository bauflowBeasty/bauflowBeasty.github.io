...
description: "Esta página te lleva desde una escena vacía hasta un guardado y una carga funcionando sin una sola línea de C#. Añadirás dos componentes, marcarás algunas c"
---
...
Ese cubo es la cosa cuyo estado quieres conservar. En tu juego real será el jugador, una puerta, un
...
- **Save Id** — un string generado de caracteres, con un botón **New** al lado. Así es como el archivo de
  guardado reconoce este objeto en particular. No lo toques. (Importa más adelante; ver el paso 9).
...
Esa es toda la configuración. Le has dicho al sistema: cuando ocurra un guardado, anota el Transform de este
objeto — su posición, su rotación, su escala.

Fíjate en la etiqueta junto a cada componente en la lista. Dice `core`, o `ugui`, o `animation`. Eso es
qué capa sabe cómo guardarlo. Si un componente muestra una advertencia en su lugar, el sistema no tiene forma de guardar
ese tipo y el guardado fallaría; desmárcalo, y lee
[converter-modules.md](/es/docs/beasty-save-system/reference/converter-modules/) para descubrir por qué.
...
El nombre del slot es solo el nombre del archivo. `slot1`, `autosave`, `quicksave` — lo que escribas. Algunos nombres son
rechazados (cualquier cosa con una barra, cualquier cosa con `..`, y los antiguos nombres de dispositivo de Windows como `CON` o
`PRN`); las razones están en [settings.md](/es/docs/beasty-save-system/guides/settings/).
...
Ese estado sobrevivió a una detención y a un reinicio porque está en disco, no en memoria. Tienes un sistema de guardado.
...
Disk**. Pulsa **Refresh**. Tu slot aparece listado. Pulsa **Open Folder** y tu explorador de archivos se abre justo
en él.
...
Poder leer tu propio archivo de guardado es una característica, no un descuido. Cuando algo no vuelve de la
manera que esperabas, abre el archivo y mira. La respuesta suele estar justo ahí.

Si prefieres que los jugadores no puedan leerlo, activa **Encrypted** en la configuración del manager — pero lee
[encryption.md](/es/docs/beasty-save-system/guides/encryption/) primero, porque es honesta sobre lo que la encriptación aquí sí y
no te da.

Guarda de nuevo desde el juego y notarás que aparece un segundo archivo: `slot1.save.bak`. Ese es el guardado
anterior, conservado automáticamente. Si un archivo de guardado alguna vez se estropea, el botón **Restore Backup** en la ventana
Save Manager devuelve el antiguo. Consulta [backups-and-corruption.md](/es/docs/beasty-save-system/guides/backups-and-corruption/).

## 9. Las dos cosas que te van a morder

Todo lo anterior funciona. Estas dos son los lugares donde la gente pierde una tarde, así que léelas ahora en vez de
después.
...
id cada vez que juegas. Por eso el guardado puede encontrar el cubo de nuevo.

Un objeto que generas en tiempo de ejecución — un enemigo, un objeto dropeado, una bala — no tiene una escena que lo recuerde. Cada
copia que `Instantiate`s obtiene un **id completamente nuevo, cada vez**. Así que su estado se escribe en el archivo de
guardado bajo un id que nunca volverá a existir, y nunca vuelve.
...
Esto es deliberado, y es buena noticia más a menudo de lo que es mala: **la conexión que hiciste en la escena
...
sombrero rojo, y la condición de sombrero-rojo debe persistir — no puedes guardar el `Sprite` en sí. Guardas algo que
