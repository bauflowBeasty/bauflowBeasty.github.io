---
title: "Logging"
description: "How to call the API, and — the part that matters — which level to pick, so that the levels still mean something six months into a project."
---
...
BeastyConsole.LogInfo("Scene initialization complete.");
```
...
```csharp
BeastyConsole.LogWarning("Missing AudioClip on SoundManager.", true, gameObject);
```
...
![One line of every level in the console, so the colours and glyphs can be compared](/docs-images/beasty-console/log-levels-sample.png)

**`Log`** — no level, no tag. A message you did not classify.

```csharp
BeastyConsole.Log("Reached the end of the tutorial.");
```
...
```csharp
BeastyConsole.LogInfo("Config file loaded.");
```
...
```csharp
BeastyConsole.LogVerbose("Serializing player data: { id=42, level=7 }");
```
...
```csharp
BeastyConsole.LogTrace("EnterState() called on StateMachine.");
```
...
```csharp
BeastyConsole.LogDebug("Inventory slots: 24, used: 7");
```
...
```csharp
BeastyConsole.LogNotice("Player reached level cap (50).");
```
...
```csharp
BeastyConsole.LogHighlight("Cutscene triggered: Ending A");
```
...
```csharp
BeastyConsole.LogCaution("Frame budget exceeded: 18ms (target 16ms).");
```
...
```csharp
BeastyConsole.LogSuccess("Save file written.");
```
...
```csharp
BeastyConsole.LogWarning("Config key 'volume' not found, using default.");
```
...
```csharp
BeastyConsole.LogError("Failed to load save slot 2: file corrupted.");
```
...
```csharp
try { LoadProfile(); }
catch (System.Exception e) { BeastyConsole.LogException(e); }
```
...
```csharp
BeastyConsole.LogError("No collider on this interactable.", true, gameObject);
```
...
BeastyConsole.LogTrace($"Path recalculated: {node.name}", debugPathfinding);
```

The line prints only when `debugPathfinding` is true. This is a per-call filter; the master switch
`IsEnabled` is separate, and it silences everything. See [Release builds](/docs/beasty-console/guides/release-builds/).
...
![Coloured messages logged without changing the level](/docs-images/beasty-console/log-color-sample.png)

```csharp
BeastyConsole.Log("Wave 4 spawned.", BeastyConsole.LogColor.Notice);
```
...
```csharp
BeastyConsole.PrintLongMessage(json, 4000, true, "save-slot-1");
```
...
The full mapping is in the [API reference](/docs/beasty-console/reference/api/).
...
- [API reference](/docs/beasty-console/reference/api/)
